...
void initCAN_0(void)
{
    ...
    CAN_0.MB[4].ID.B.ID_STD = 0x555;     /* Expected ID from logger */
    CAN_0.MB[4].CS.B.CODE = 4;           /* RX EMPTY (ready to receive) */
    CAN_0.RXMGMASK.R = 0x1FFFFFFF;       /* Accept all bits for filtering */

    /* ---------- Configure TX mailbox (MB0) ---------- */
    CAN_0.MB[0].CS.B.CODE = 8;           /* TX INACTIVE (ready for use) */
...
/* ------------------------------------------------------------- */
/* Wait for frame, modify ID, transmit back                      */
/* ------------------------------------------------------------- */
void CAN_Receive_And_ReTransmit(void)
{
    ...
    /* ---- Modify ID and transmit back ---- */
    CAN_0.MB[0].CS.B.IDE = 0;           /* Standard ID */
    CAN_0.MB[0].ID.B.ID_STD = 0x666;    /* New ID for retransmission */
    CAN_0.MB[0].CS.B.RTR = 0;           /* Data frame */
    CAN_0.MB[0].CS.B.DLC = RxLENGTH;    /* Same length */

    for (j = 0; j < RxLENGTH; j++) {
        CAN_0.MB[0].DATA.B[j] = RxDATA[j]; /* Copy same data */
    }

    CAN_0.MB[0].CS.B.SRR = 1;          /* Tx frame */
    CAN_0.MB[0].CS.B.CODE = 0xC;       /* Activate transmission */
